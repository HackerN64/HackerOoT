...
s16 CutsceneManager_SetHudVisibility(s16 csHudVisibility) {
    ...
    switch (csHudVisibility) {
        ...
        case CS_HUD_VISIBILITY_A_HEARTS_MAGIC:
            hudVisibility = HUD_VISIBILITY_A_HEARTS_MAGIC_FORCE;
            break;
    ...
    Interface_ChangeHudVisibilityMode(hudVisibility);
...
void CutsceneManager_End(void) {
    ...
    switch (sCutsceneMgr.startMethod) {
        case CS_START_2:
            ...
            FALLTHROUGH;
        case CS_START_1:
            Player_SetCsActionWithHaltedActors(sCutsceneMgr.play, NULL, PLAYER_CSACTION_7); // PLAYER_CSACTION_END
    ...
    switch (csEntry->endCam) {
            ...
            //     RET_CAM->stateFlags =
            //         (RET_CAM->stateFlags & ~CAM_STATE_CAMERA_IN_WATER) | (CUR_CAM->stateFlags &
            //         CAM_STATE_CAMERA_IN_WATER);
        ...
        case CS_END_CAM_1:
            ...
            if (sCutsceneMgr.isCameraStored) {
                ...
                RET_CAM->stateFlags = (RET_CAM->stateFlags & ~CAM_STATE_CAMERA_IN_WATER) |
                                      (CUR_CAM->stateFlags & CAM_STATE_CAMERA_IN_WATER);

                RET_CAM->stateFlags =
                    (RET_CAM->stateFlags & ~CAM_STATE_CHECK_BG) | (oldStateFlags & CAM_STATE_CHECK_BG);
                sCutsceneMgr.isCameraStored = false;
            }
    ...
    if (sCutsceneMgr.subCamId != SUB_CAM_ID_DONE) {
        ...
        Play_ChangeCameraStatus(sCutsceneMgr.play, sCutsceneMgr.retCamId, CAM_STAT_ACTIVE);
    }
...
s16 CutsceneManager_StartWithPlayerCs(s16 csId, Actor* actor) {
    ...
    if (startCsId > CS_ID_NONE) {
        Player_SetCsActionWithHaltedActors(sCutsceneMgr.play, NULL, PLAYER_CSACTION_8); // PLAYER_CSACTION_WAIT
        if (sCutsceneMgr.length == 0) {
            CutsceneManager_Stop(sCutsceneMgr.csId);
        }
...
s16 CutsceneManager_StartWithPlayerCsAndSetFlag(s16 csId, Actor* actor) {
    ...
    if (startCsId > CS_ID_NONE) {
        Player_SetCsActionWithHaltedActors(sCutsceneMgr.play, NULL, PLAYER_CSACTION_8);
        if (sCutsceneMgr.length == 0) {
            CutsceneManager_Stop(sCutsceneMgr.csId);
        }
        if (actor != NULL) {
    ...
    if (csType != 0) {
        ...
        if ((retCam->setting == CAM_SET_START0) || /* (retCam->setting == CAM_SET_START2) || */
            ...
            if (CutsceneManager_FindEntranceCsId() != csId) {
                Camera_800E0348(retCam);
            } else {
                Camera_UnsetStateFlag(retCam, CAM_STATE_CHECK_BG);
            }
        }
        ...
        Play_ChangeCameraStatus(sCutsceneMgr.play, sCutsceneMgr.retCamId, CAM_STAT_WAIT);
        Play_ChangeCameraStatus(sCutsceneMgr.play, sCutsceneMgr.subCamId, CAM_STAT_ACTIVE);
...
s16 CutsceneManager_FindEntranceCsId(void) {
    ...
    for (csId = 0; csId < sSceneCutsceneCount; csId++) {
        if ((sSceneCutsceneList[csId].customValue >= 100) &&
            (sSceneCutsceneList[csId].customValue == (sCutsceneMgr.play->spawn + 100))) {
            return csId;
        }
    }
...
s32 CutsceneManager_800F22C4(s16 csId, Actor* actor) {
    f32 dist;
    s16 screenPosX;
    s16 screenPosY;
