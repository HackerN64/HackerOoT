...
void BgCheck_Allocate(CollisionContext* colCtx, PlayState* play, CollisionHeader* colHeader) {
    ...
    if ((R_SCENE_CAM_TYPE == SCENE_CAM_TYPE_FIXED_SHOP_VIEWPOINT) ||
        ...
        if (play->sceneId == SCENE_STABLE) {
            ...
            PRINTF2("/* BGCheck LonLonサイズ %dbyte */\n", 0x3520);
            colCtx->memSize = 0x3520;
        } else {
            // "/* BGCheck Mini Size %dbyte */\n"
            PRINTF2("/* BGCheck ミニサイズ %dbyte */\n", 0x4E20);
        ...
        PRINTF2("/* BGCheck Spot用サイズ %dbyte */\n", 0xF000);
...
CollisionHeader* BgCheck_GetCollisionHeader(CollisionContext* colCtx, s32 bgId) {
    ...
    if (!(colCtx->dyna.bgActorFlags[bgId] & BGACTOR_IN_USE)) {
        ...
        PRINTF2("T_BGCheck_getBGDataInfo():そのbg_actor_indexは使われておりません。index=%d\n");
        PRINTF2(VT_RST);
        return NULL;
    }
    ...
    *outPoly = NULL;
    ...
    while (true) {
        ...
        if (BgCheck_PosErrorCheck(&checkPos, "../z_bgcheck.c", 4410)) {
            if (actor != NULL) {
                PRINTF2("こいつ,pself_actor->name %d\n", actor->id);
            }
        }
#endif
...
s32 DynaPoly_SetBgActor(PlayState* play, DynaCollisionContext* dyna, Actor* actor, CollisionHeader* colHeader) {
    ...
    if (!foundSlot) {
        PRINTF2(VT_FGCOL(RED));
        PRINTF2("DynaPolyInfo_setActor():ダイナミックポリゴン 空きインデックスはありません\n");
        PRINTF2(VT_RST);
        return BG_ACTOR_MAX;
    }
    ...
    PRINTF2(VT_FGCOL(GREEN));
    PRINTF2("DynaPolyInfo_setActor():index %d\n", bgId);
    PRINTF2(VT_RST);
    return bgId;
}
...
void DynaPoly_DeleteBgActor(PlayState* play, DynaCollisionContext* dyna, s32 bgId) {
    ...
    PRINTF2(VT_FGCOL(GREEN));
    PRINTF2("DynaPolyInfo_delReserve():index %d\n", bgId);
    PRINTF2(VT_RST);
    if (!DynaPoly_IsBgIdBgActor(bgId)) {
...
void DynaPoly_UpdateContext(PlayState* play, DynaCollisionContext* dyna) {
    ...
    for (i = 0; i < BG_ACTOR_MAX; i++) {
        if (dyna->bgActorFlags[i] & BGACTOR_1) {
            // Initialize BgActor
            PRINTF2(VT_FGCOL(GREEN));
            PRINTF2("DynaPolyInfo_setup():削除 index=%d\n", i);
            PRINTF2(VT_RST);
