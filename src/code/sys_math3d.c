...
void Math3D_LineClosestToPoint(InfiniteLine* line, Vec3f* pos, Vec3f* closestPoint) {
    ...
    if (IS_ZERO(dirVectorLengthSq)) {
        ...
        PRINTF2("Math3D_lineVsPosSuisenCross():直線の長さがありません\n");
        PRINTF2("cross = pos を返します。\n"); // "Returns cross = pos."
        PRINTF2(VT_RST);
...
f32 Math3D_UDistPlaneToPos(f32 nx, f32 ny, f32 nz, f32 originDist, Vec3f* p) {
    if (IS_DEBUG && IS_ZERO(sqrtf(SQ(nx) + SQ(ny) + SQ(nz)))) {
        ...
        PRINTF2("Math3DLengthPlaneAndPos():法線size がゼロ近いです%f %f %f\n", nx, ny, nz);
        PRINTF2(VT_RST);
        return 0.0f;
    }
...
f32 Math3D_DistPlaneToPos(f32 nx, f32 ny, f32 nz, f32 originDist, Vec3f* p) {
    ...
    if (IS_ZERO(normMagnitude)) {
        ...
        PRINTF2("Math3DSignedLengthPlaneAndPos():法線size がゼロ近いです%f %f %f\n", nx, ny, nz);
        PRINTF2(VT_RST);
        return 0.0f;
    }
