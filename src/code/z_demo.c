...
void Cutscene_UpdateScripted(PlayState* play, CutsceneContext* csCtx) {
    ...
    u8 canStartCutscene =
        (CHECK_BTN_COMBO(CS_CTRL_USE_BTN_COMBO, input, CS_CTRL_BTN_HOLD_FOR_COMBO, CS_CTRL_RESTART_CONTROL) &&
...
void CutsceneCmd_Destination(PlayState* play, CutsceneContext* csCtx, CsCmdDestination* cmd) {
    ...
    runCmdDestination =
        skipTitleScreenCS &&
        (runCmdDestination || ((csCtx->curFrame > 20) &&
                               CHECK_BTN_COMBO(CS_CTRL_USE_BTN_COMBO, &play->state.input[CS_CTRL_CONTROLLER_PORT],
                                               CS_CTRL_BTN_HOLD_FOR_COMBO, CS_CTRL_RUN_DEST_CONTROL) &&
...
#ifdef FW_SPLIT_AGE
                fwBackup = gSaveContext.save.info.fwMain;
                gSaveContext.save.info.fwMain = gSaveContext.save.info.fwSecondary;
                gSaveContext.save.info.fwSecondary = fwBackup;
#else
...
void Cutscene_ProcessScript(PlayState* play, CutsceneContext* csCtx, u8* script) {
    ...
    if (CHECK_BTN_COMBO(CS_CTRL_USE_BTN_COMBO, &play->state.input[CS_CTRL_CONTROLLER_PORT], CS_CTRL_BTN_HOLD_FOR_COMBO,
                        CS_CTRL_STOP_CONTROL)) {
        csCtx->state = CS_STATE_STOP;
        return;
    }
#endif
