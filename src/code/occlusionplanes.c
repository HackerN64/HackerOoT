...
static s32 OcclusionPlane_Choose(PlayState* play, Vec3f* selCandidate){
    ...
    camDir.x *= len2; camDir.y *= len2; camDir.z *= len2;
    ...
    while(true){
        ...
        if(!lookingAway){
            for(s32 i=0; i<4; ++i){
                ...
                if(len2 > 0.0f) len2 = 1.0f / sqrtf(len2);
                tangent.x *= len2; tangent.y *= len2; tangent.z *= len2;
                ...
                if(dist > maxDistProjView) maxDistProjView = dist;
            }
        }
