...
extern s32 osPiStartDma(OSIoMesg* mb, s32 priority, s32 direction, u32 devAddr, void* dramAddr, u32 size,
                        OSMesgQueue* mq);
...
static regDesc causeDesc[] = { { CAUSE_BD, CAUSE_BD, "BD" },
...
// Overwrite osSyncPrintf
    ...
    osCreateThread(&usbThread, USB_THREAD_ID, debug_thread_usb, 0, (usbThreadStack + USB_THREAD_STACK / sizeof(u64)),
                   USB_THREAD_PRI);
...
// Initialize the remote debugger thread
    ...
    osCreateThread(&rdbThread, RDB_THREAD_ID, debug_thread_rdb, (void*)osGetThreadId(NULL),
                   (rdbThreadStack + RDB_THREAD_STACK / sizeof(u64)), RDB_THREAD_PRI);
    ...
    switch (exc->code) {
        case EXCEPTION_CODE_BREAKPOINT:
            ...
            if (debug_ismanualpause) {
                ...
                exc->regs->epc += 4; // Gotta increment PC otherwise the program will likely hit the breakpoint again in
                                     // debug_initialize
            }
            break;
        default:
    ...
    if (addr >= 0x80000000 && addr < 0x80000000 + osMemSize) {
        for (i = 0; i < BPOINT_COUNT; i++) {
            ...
            if (debug_bpoints[i].addr == NULL) {
                // Store the address and the instruction (the value in its memory) before we overwrite it with a
                // breakpoint
                ...
                // So since we have a range of values, we can encode the index into the instruction itself, in the
                // middle 20 bits
