...
void DemoGeff_Init(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.params < 0 || this->actor.params >= 9) {
        PRINTF2(VT_FGCOL(RED) "Demo_Geff_Actor_ct:arg_dataがおかしい!!!!!!!!!!!!\n" VT_RST);
...
void func_80978370(DemoGeff* this, PlayState* play) {
    ...
    if (initFunc == NULL) {
        PRINTF2(VT_FGCOL(RED) " Demo_Geff_main_init:初期化処理がおかしいarg_data = %d!\n" VT_RST, params);
        Actor_Kill(&this->actor);
        return;
    }
    initFunc(this, play);
...
void func_809783D4(DemoGeff* this, PlayState* play) {
    ...
    if (objectSlot < 0) {
        PRINTF2(VT_FGCOL(RED) "Demo_Geff_main_bank:バンクを読めない arg_data = %d!\n" VT_RST, params);
        Actor_Kill(thisx);
        return;
    }
